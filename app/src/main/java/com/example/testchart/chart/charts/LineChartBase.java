...
import com.example.testchart.chart.listeners.LineChartTouchListener;
import com.example.testchart.chart.renderer.XAxisRenderer;
import com.example.testchart.chart.renderer.YAxisRenderer;
...
public abstract class LineChartBase<T extends BaseLineData<? extends
        ILineDataSet<? extends Entry>>>
        extends Chart<T> implements BaseLineDataProvider {
     ...
     */
    private final int mMaxVisibleCount = 100;

    /**
     * flag that indicates if auto scaling on the y axis is enabled
     */
    private final boolean mAutoScaleMinMaxEnabled = false;
     ...
     * separately
     */
    private boolean mPinchZoomEnabled = false;
    ...
    private final boolean mClipValuesToContent = false;

    /**
     * Sets the minimum offset (padding) around the chart, defaults to 15
     */
    private float mMinOffset = 15.f;

    /**
     * flag indicating if the chart should stay at the same position after a rotation. Default is false.
     */
    private boolean mKeepPositionOnRotation = false;
    ...
    private YAxisRenderer mAxisRendererLeft;
    private YAxisRenderer mAxisRendererRight;
    ...
    protected void init() {
        super.init();
        Log.d(TAG, "BarLineChartBase init: ");
        ...
        mChartTouchListener = new LineChartTouchListener(this, mViewPortHandler.getMatrixTouch(), 3f);
    ...
    private void prepareOffsetMatrix() {
    ...
    private void calculateLegendOffsets(RectF offsets) {
    ...
    public void computeScroll() {
        ...
        if (mChartTouchListener instanceof LineChartTouchListener)
            ((LineChartTouchListener) mChartTouchListener).computeScroll();
    }
    ...
    private final Matrix mZoomMatrixBuffer = new Matrix();
     ...
     * @return
     */
    private float getAxisRange(YAxis.AxisDependency axis) {
    ...
    private void getValuesByTouchPoint(float x, float y, YAxis.AxisDependency axis, MPPointD outputPoint) {
     ...
     * @return
     */
    public ILineDataSet getDataSetByTouchPoint(float x, float y) {
        Highlight h = getHighlightByTouchPoint(x, y);
        if (h != null) {
     ...
     */
    private final MPPointD posForGetLowestVisibleX = MPPointD.getInstance(0, 0);
     ...
     */
    private final MPPointD posForGetHighestVisibleX = MPPointD.getInstance(0, 0);
    ...
    /**
     * Returns the y-axis object to the corresponding YAxis.AxisDependency. In the
     * horizontal bar-chart, LEFT == top, RIGHT == BOTTOM
     *
     * @param axis
     * @return
     */
    private YAxis getAxis(YAxis.AxisDependency axis) {
        if (axis == YAxis.AxisDependency.LEFT)
            return mAxisLeft;
        else
            return mAxisRight;
    ...
    private final float[] mOnSizeChangedBuffer = new float[2];
